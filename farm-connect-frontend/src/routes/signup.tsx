...
function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (role === "driver") {
      ...
      if (!govtIdNumber.trim() || (!govtIdProofName && !govtIdNumber)) {
        setError("Government-issued ID proof and number are required.");
        return;
      }
      if (!licenseNumber.trim() || (!licenseProofName && !licenseNumber)) {
      ...
      const { data, error: signUpErr } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            ...
            phone: phone.trim(),
            role,
            isVerified: role === "driver" ? true : true, // Activated upon document verification
            ...(role === "driver" && {
